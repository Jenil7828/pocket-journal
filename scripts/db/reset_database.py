#!/usr/bin/env python3
"""
Complete database reset script for Pocket Journal
This script will:
1. Delete all existing collections and documents
2. Recreate the database with proper structure
3. Optionally migrate sample data
"""
...
def reset_database():
    """Reset the entire Firebase database"""
    ...
    print("⚠️  WARNING: This will delete ALL data in your Firebase database!")
    ...
    if add_sample == 'y':
        ...
        run_migration(db, IST)
    
    print("\n✅ Database reset and setup complete!")

def run_migration(db, IST):
    """Run the migration script to populate database with sample data from MySQL dump"""
    
    print("\n📝 Running migration script...")
    
    # Sample user UID (replace with your actual UID)
    USER_UID = "t6xSqQyHIEQzeDsL16qdKlDPQU42"
    ...
    # Upload Journal Entries with proper structure
    ...
    for entry in journal_entries_data:
        ...
        doc_ref.set({
            ...
            "updated_at": created_at  # Set updated_at = created_at for migrated entries
        })
        entry_ids.append(doc_ref.id)
    
    print(f"✅ Added {len(entry_ids)} journal entries")
    
    # Upload Entry Analysis
    print("🔍 Adding entry analysis...")
    ...
    print(f"✅ Added {len(entry_analysis_data)} entry analyses")
    print("🎉 Migration completed: all 30 entries + analyses uploaded with proper structure!")

def add_sample_data(db, IST):
    """Add sample data to the reset database"""
    
    print("\n📝 Adding sample data...")
    
    # Sample user UID (replace with your actual UID)
    sample_uid = "t6xSqQyHIEQzeDsL16qdKlDPQU42"
    
    # Sample journal entries
    sample_entries = [
        {
            "entry_text": "Woke up early and did a 30-minute morning run. Felt energized and ready for the day!",
            "created_at": datetime.now(IST).replace(hour=7, minute=30),
            "updated_at": datetime.now(IST).replace(hour=7, minute=30)
        },
        {
            "entry_text": "Had a productive workday. Completed the project ahead of schedule. Feeling accomplished!",
            "created_at": datetime.now(IST).replace(hour=18, minute=0),
            "updated_at": datetime.now(IST).replace(hour=18, minute=0)
        },
        {
            "entry_text": "Spent evening with friends. Had great conversations and lots of laughter. Feeling grateful for good relationships.",
            "created_at": datetime.now(IST).replace(hour=21, minute=30),
            "updated_at": datetime.now(IST).replace(hour=21, minute=30)
        }
    ]
    ...
    for entry_data in sample_entries:
        doc_ref = db.collection("journal_entries").document()
        entry_data["uid"] = sample_uid
        doc_ref.set(entry_data)
        entry_ids.append(doc_ref.id)
        print(f"✅ Added entry: {entry_data['entry_text'][:50]}...")
    
    # Add corresponding analysis
    sample_analyses = [
        {
            "summary": "Morning run provided energy and positive start to the day.",
            "mood": {
                "happy": 0.85,
                "sad": 0.05,
                "angry": 0.02,
            ...
            "created_at": datetime.now(IST).replace(hour=7, minute=35)
        },
        {
            "summary": "Productive workday with successful project completion.",
            "mood": {
                "happy": 0.75,
                "sad": 0.05,
                "angry": 0.05,
            ...
            "created_at": datetime.now(IST).replace(hour=18, minute=5)
        },
        {
            "summary": "Evening with friends brought joy and gratitude.",
            "mood": {
                "happy": 0.90,
                "sad": 0.02,
                "angry": 0.01,
            ...
            "created_at": datetime.now(IST).replace(hour=21, minute=35)
        }
    ]
    
    for i, analysis_data in enumerate(sample_analyses):
        doc_ref = db.collection("entry_analysis").document()
        analysis_data["entry_id"] = entry_ids[i]
        doc_ref.set(analysis_data)
        print(f"✅ Added analysis for entry {i+1}")
    ...
    sample_insight = {
        ...
        "goals": [
            {
                ...
                "description": "Continue morning exercise routine to start days with energy"
            },
            {
        ...
        "progress": "Good progress on maintaining energy through exercise and building relationships through social activities.",
        "negative_behaviors": "No significant negative patterns identified in this period.",
        "remedies": "1. Continue current exercise routine. 2. Schedule regular social activities. 3. Maintain work-life balance.",
        "appreciation": "Strong commitment to health and relationships. Good work-life balance maintained.",
        "conflicts": "No major conflicts identified in this period.",
        "raw_response": '{"goals": [...], "progress": "...", ...}',
        "created_at": datetime.now(IST)
    }
    
    insight_doc_ref = db.collection("insights").document()
    insight_doc_ref.set(sample_insight)
    
    # Add insight-entry mappings
    for entry_id in entry_ids:
        db.collection("insight_entry_mapping").add({
            "insight_id": insight_doc_ref.id,
            "entry_id": entry_id
        })
    
    print(f"✅ Added sample insight with {len(entry_ids)} entry mappings")
    print(f"\n🎉 Sample data added successfully!")
    print(f"📊 Added: {len(sample_entries)} entries, {len(sample_analyses)} analyses, 1 insight")
...
if __name__ == "__main__":
    print("🔄 Pocket Journal Database Reset Tool")
    print("=" * 50)
    ...
    try:
        ...
        if not firebase_admin._apps:
            ...
            firebase_admin.initialize_app(cred)
        db = firestore.client()
        verify_database_structure(db)
        
    except Exception as e:
        print(f"❌ Error during database reset: {str(e)}")
        print("Please check your .env file and Firebase credentials.")
