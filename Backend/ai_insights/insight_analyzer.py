...
from ai_insights.gaurds import setup_gemini_env
...
class InsightsGenerator:
    def __init__(
        ...
        if not gemini["enabled"]:
            raise RuntimeError("Gemini is disabled or missing API key")
    ...
    def parse_gemini_response(raw_text: str) -> Dict:
        """
        Defensive JSON extractor for Gemini responses.
    ...
    def generate_insights(
        ...
        for i in range(0, len(entries), self.batch_size):
            ...
            batch_text = "\n".join(
                f"Date: {e['created_at']}\n"
                f"Entry: {e.get('entry_text', '')}\n"
                f"Analysis: {e.get('analysis', {}).get('summary', '')}"
                for e in batch
            )

            # prompt = self.prompt_template.format(entries=batch_text)
...
# from Backend.persistence.db_manager import DBManager
# from Backend.ai_insights.gaurds import setup_gemini_env


# class InsightsGenerator:
#     def __init__(
#             self, db: DBManager, model_name="gemini-2.0-flash", temperature=0.7, batch_size=20):
#         self.db = db
